...
//    }
    ...
    //7.	Eliminar problemabarial en BD.
    @Test
    public void given_problemaBarrial_when_delete_then_ok(){
        // Arrange: Creamos el mock del DAO
        IProblemaDAO mockDAO = Mockito.mock(IProblemaDAO.class);
        Problema problema = new Problema("Basura", "Acumulación de basura", 0, "img");
        problema.setId(5);
        // Simulamos que el DAO retorna el problema cuando se busca por ID
        when(mockDAO.buscarProblema(5)).thenReturn(problema);
        // Asignar mock al service
        ProblemaService.setProblemaDAO(mockDAO);
        // Act: Llamamos al método eliminarProblema
        ProblemaService.eliminarProblema(5);
        // Assert: Verificamos que el método eliminarProblema fue invocado con el problema correcto
        verify(mockDAO, times(1)).eliminarProblema(problema);
    }

    //8.	Editar campos de problemabarial en BD.
    @Test
    public void given_problemaBarrial_when_update_then_ok(){
